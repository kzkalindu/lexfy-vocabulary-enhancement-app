...
class _LeaderboardScreenState extends State<LeaderboardScreen> {
  ...
  // Backend API URL
  final String backendUrl = "https://lexfy-vocabulary-enhancement-app.onrender.com/api/leaderboard";
  ...
  try {
    ...
    // Make HTTP GET request to backend with corrected URL
    final response = await http.get(Uri.parse('$backendUrl/api/leaderboard'));
    

    if (response.statusCode == 200) {
      // Parse JSON response - handle the object with 'leaderboard' property
  ...
  void _checkCurrentUserDetails() {
    // Assuming the backend returns the current user's email in the response
    final currentUserEmail = "user@example.com"; // Replace with actual current user email
      ...
      children: [
        ...
        ...List.generate(
          ...
          (index) {
            ...
            final user = filteredLeaderboard[userIndex];
            final isCurrentUser = user['email'] == "user@example.com"; // Replace with actual current user email
