...
class _SelectTaskPageState extends State<SelectTaskPage> {
  final UserService _userService = UserService();
  int _currentLevel = 0;
  int _streak = 0; // New field for streak
  bool _isLoading = true;
  ...
  Future<void> _loadUserData() async {
    try {
      ...
      if (mounted) {
        setState(() {
          _currentLevel = _userService.user.currentLevel;
          _streak = _userService.user.streak;
          _isLoading = false;
        });
      }
    } catch (e) {
      if (mounted) {
        setState(() {
          _currentLevel = 1; // Fallback to level 1
          _streak = 0; // Fallback to 0
          _isLoading = false;
        });
        ScaffoldMessenger.of(context).showSnackBar(
  ...
  Widget build(BuildContext context) {
    ...
    final Size screenSize = MediaQuery.of(context).size;
    final Color primaryPurple = const Color(0xFF636AE8);
    final Color lightPurple = const Color(0xFF8E95F0);
    final Color darkPurple = const Color(0xFF3B46C4);
    final Color veryLightPurple = const Color(0xFFD1D5FA);
            ...
            child: Column(
              ...
              children: [
                ...
                _buildTaskCard(
                  context,
                  title: 'Daily Word Challenge',
                  subtitle: 'Your Daily Streak: $_streak Days',
                  iconData: Icons.calendar_today,
                  primaryColor: primaryPurple,
                  lightColor: lightPurple,
                  darkColor: darkPurple,
