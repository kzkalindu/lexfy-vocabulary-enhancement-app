...
class _DailyChallengeScreenState extends State<DailyChallengeScreen> {
  ...
  Future<void> _loadUserData() async {
    try {
      ...
      if (mounted) {
        setState(() {
          lastAnswerDate = _userService.user.lastStreakDate; // Assuming this field is added
          _checkStreakReset();
          isAnsweredToday = lastAnswerDate != null && _isSameDay(lastAnswerDate!, DateTime.now());
        });
      }
    } catch (e) {
      if (mounted) {
  ...
  Future<void> _resetStreak() async {
    try {
      await _userService.updateUserProgress(streak: 0, lastStreakDate: null);
      if (mounted) {
        setState(() {
          lastAnswerDate = null;
        });
  ...
  Future<void> _updateStreak(bool isCorrect) async {
    ...
    if (lastAnswerDate == null || !_isSameDay(lastAnswerDate!, now)) {
      int newStreak = _userService.user.streak;
      if (isCorrect) {
        if (lastAnswerDate == null) {
          ...
          newStreak = 1; // Missed a day, restart
        }
      } else if (lastAnswerDate != null && now.difference(lastAnswerDate!).inDays > 1) {
        newStreak = 0; // Missed a day, reset
      }
      ...
      try {
        ...
        if (mounted) {
          setState(() {
            errorMessage = 'Error updating streak: $e';
          });
        }
      }
    }
