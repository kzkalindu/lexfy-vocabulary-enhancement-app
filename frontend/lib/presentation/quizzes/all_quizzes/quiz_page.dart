...
class _QuizPageState extends State<QuizPage> {
  ...
  Future<void> _initialize() async {
    try {
      ...
      _quizzes = await _quizService.getQuizzesByLevel(_userService.user.currentLevel);
      setState(() {
        _isLoading = false;
      });
    } catch (e) {
  ...
  void _submitAnswer() {
    if (_selectedAnswer != null && !_isAnswered) {
      setState(() {
        ...
        if (_selectedAnswer == _quizzes[_currentQuizIndex].options[_quizzes[_currentQuizIndex].answerIndex]) {
          _correctAnswers++;
          _userService.updateUserProgress(xpPoints: _userService.user.xpPoints + 10);
        } else {
          _userService.updateUserProgress(xpPoints: _userService.user.xpPoints + 2);
        }
      });
    }
  }
  ...
  void _checkLevelCompletion() {
    if (_correctAnswers >= 4) {
      _userService.updateUserProgress(
        currentLevel: _userService.user.currentLevel + 1,
        xpPoints: _userService.user.xpPoints + 50,
        completedLevels: [..._userService.user.completedLevels, _userService.user.currentLevel],
      ).then((_) {
        setState(() {
          _currentQuizIndex = 0;
          _correctAnswers = 0;
        ...
        _initialize();
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Level Completed!')));
      });
    } else {
      setState(() {
        _currentQuizIndex = 0;
  ...
  Widget build(BuildContext context) {
    ...
    if (_quizzes.isEmpty) {
      return Scaffold(body: Center(child: Text('No quizzes available')));
    }

    final currentQuiz = _quizzes[_currentQuizIndex];
    return Scaffold(
      appBar: AppBar(
        title: Text('Level ${_userService.user.currentLevel}'),
      ),
      body: Padding(
        padding: EdgeInsets.all(16.0),
        child: Column(
          children: [
            Text('Question ${_currentQuizIndex + 1}/${_quizzes.length}'),
            SizedBox(height: 20),
            Text(currentQuiz.question, style: TextStyle(fontSize: 18)),
            SizedBox(height: 20),
            ...currentQuiz.options.map((option) => RadioListTile<String>(
              title: Text(option),
              value: option,
              groupValue: _selectedAnswer,
            ...
            SizedBox(height: 20),
            ElevatedButton(
              onPressed: _selectedAnswer == null ? null : (_isAnswered ? _nextQuestion : _submitAnswer),
            ...
            if (_isAnswered)
              Text(
                _selectedAnswer == currentQuiz.options[currentQuiz.answerIndex]
                    ? 'Correct!'
                    : 'Wrong. Correct answer: ${currentQuiz.options[currentQuiz.answerIndex]}',
                style: TextStyle(color: _selectedAnswer == currentQuiz.options[currentQuiz.answerIndex] ? Colors.green : Colors.red),
              ),
          ],
        ),
      ),
