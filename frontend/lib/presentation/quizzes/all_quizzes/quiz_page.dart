...
class _QuizPageState extends State<QuizPage> {
  ...
  bool _isAnswered = false;
  int _correctAnswers = 0;
  int _currentLevel = 1; // User's actual progress level
  int _playingLevel = 1; // Level currently being played
  bool _showWelcome = true;
  bool _showLevelSelection = false;
  bool _hasError = false;
  String _errorMessage = '';
  ...
  Future<void> _loadQuizzes() async {
    ...
    try {
      _quizData = await _quizService.getQuizzesByLevel(
        _playingLevel,
        _userService.user.usedQuizzes,
      );
      if (_quizData.isEmpty) {
        if (_playingLevel > 1) {
          ...
          if (_quizData.isEmpty) {
            throw Exception('No new quizzes available for Level $_playingLevel');
          } else {
            ScaffoldMessenger.of(context).showSnackBar(
              SnackBar(
                content: Text(
                    'No new quizzes for Level ${_playingLevel + 1}. Showing Level $_playingLevel instead.'),
      ...
      _quizzes = _quizData.map((data) => data['quiz'] as Quiz).toList();
      if (!mounted) return;
      setState(() {
        _isLoading = false;
        _showWelcome = true;
  ...
  void _submitAnswer() {
    if (_selectedAnswer != null && !_isAnswered) {
      setState(() {
        _isAnswered = true;
        if (_selectedAnswer ==
            _quizzes[_currentQuizIndex].options[_quizzes[_currentQuizIndex].answerIndex]) {
          _correctAnswers++;
          print('Correct answer - Calling _updateXP with ${Constants.XP_CORRECT_ANSWER}');
          _updateXP(Constants.XP_CORRECT_ANSWER);
        } else {
          print('Wrong answer - Calling _updateXP with ${Constants.XP_WRONG_ANSWER}');
          _updateXP(Constants.XP_WRONG_ANSWER);
        }
      });
    }
  ...
  void _checkLevelCompletion() {
    if (_correctAnswers >= 4) {
      print('Level completed - Updating XP with ${Constants.XP_PASS_LEVEL}');
      _updateXP(Constants.XP_PASS_LEVEL);

      List<String> newUsedQuizzes = [
        ..._userService.user.usedQuizzes,
        ..._quizData.map((data) => data['id'] as String),
      ];

      if (_playingLevel == _currentLevel) {
        final newLevel = _currentLevel + 1;
        print('Level completed - Updating progress to level $newLevel');
        _userService.updateUserProgress(
          currentLevel: newLevel,
          completedLevels: [..._userService.user.completedLevels, _currentLevel],
          usedQuizzes: newUsedQuizzes,
        );
        if (!mounted) return;
        setState(() {
          _currentLevel = newLevel;
          _playingLevel = newLevel;
        });
      } else {
        _userService.updateUserProgress(
          usedQuizzes: newUsedQuizzes,
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text('Level $_playingLevel Completed!'),
          backgroundColor: Colors.green,
        ),
      );
    } else {
  ...
  void _updateXP(int xp) {
    if (_auth.currentUser != null) {
      print('Updating XP - Current XP: ${_userService.user.xpPoints}, Adding: $xp');
      _userService.updateUserProgress(xpPoints: _userService.user.xpPoints + xp);
      print('XP Update called - New XP should be: ${_userService.user.xpPoints}');
    } else {
      print('No user logged in - XP update skipped');
    }
  }
                ...
                return GestureDetector(
                  ...
                  child: Container(
                    decoration: BoxDecoration(
                      ...
                      boxShadow: [
                        BoxShadow(
                          color: (isCurrentLevel ? Colors.pink : Colors.deepPurple)
                              .withOpacity(0.3),
                          blurRadius: 8,
                          offset: Offset(0, 3),
                        ),
                ...
                child: Container(
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      SizedBox(height: 20),
                      Text(
                        'Would you like to try a different level or retry?',
                        textAlign: TextAlign.center,
                        style: TextStyle(
                          fontSize: 14,
                ...
                child: Center(
                  child: Container(
                    ...
                    child: Column(
                      ...
                      children: [
                        Text(
                          'Welcome to Level ${_playingLevel.toString().padLeft(2, '0')}!',
                          style: TextStyle(
                            fontSize: 24,
                            fontWeight: FontWeight.bold,
                            color: Colors.deepPurple,
              ...
              Expanded(
                child: Center(
                  child: Container(
                    ...
                    child: Text(
                      'No new quizzes available for Level $_playingLevel',
                      style: TextStyle(fontSize: 18),
                    ),
                  ),
                ),
                      ...
                      Row(
                        ...
                        children: [
                          ...
                          Container(
                            ...
                            child: Text(
                              'Level ${_playingLevel.toString().padLeft(2, '0')}',
                              style: TextStyle(
                                color: Colors.deepPurple,
                                fontWeight: FontWeight.bold,
                                fontSize: 14,
